...
c      write(*,*)'before ncvdef2(i_salphal)'

      vid=ncvdef2(ncid,'i_salphal',NCLONG,1,nbulkid,istatus)
      call ncaptc2(ncid,vid,'long_name',NCCHAR,91,
     +     'Species contributed for linear damping wavenumber: '//
     +     '=0 no contribution, =1 with contribution',istatus)
...
c      write(*,*)'wrtnetcdf after ncptc charge istatus',istatus
      ...
      call ncvid2(vid,ncid,'i_salphal',istatus)
      call ncvpt_int2(ncid,vid,1,nbulk,i_salphal,istatus)
c      write(*,*)'wrtnetcdf after ncptc i_salphal istatus',istatus
...
cShizhe Yang Apr.16.2025 iabsorp condition ccommented
!       if ((iabsorp.eq.3.or.iabsorp.eq.9).or.
...
c-----define variables and attributes
     ...
     +           'erg/sec',istatus)
      ...
      ! if (((iabsorp.eq.3).or.(iabsorp.eq.2)) .and. nbulk.gt.1) then
