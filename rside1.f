...
c      dimension vp(nbulka),wp(nbulka)
...
c         write(*,*)'in rside1 z,r,phi,cnz,cnr,cm',
c     &                        z,r,phi,cnz,cnr,cm
c         write(*,*)'rside1 rho,idif,id,ioxm',rho,idif,id,ioxm
...
c           write(*,*)'rside1 z,r,phi,rho,cnr,cm',
c     &     z,r,phi,rho,cnr,cm
c           write(*,*)'cnz,hnz,cnzplus,cnzminus',
c     &     cnz,hnz,cnzplus,cnzminus
...
c           write(*,*)'rside1 cnzplus,hp',cnzplus,hp
c           write(*,*)'rside1 cnzmins,hp',cnzmins,hm
c           write(*,*)'rside1 dddcnz deru(1)',dddcnz,deru(1)     
...
c          write(*,*)'rside1 cnrplus,hp'
c          write(*,27)cnrplus,hp
c          write(*,*)'rside1 cnrmins,hm'
c          write(*,27)cnrmins,hm
c          write(*,*)'hp-hm , 2.d0*hnr'
c          write(*,27)(hp-hm),2.d0*hnr
c           write(*,*)'rside1 dddcnr,deru(2)'
c           write(*,27)dddcnr,deru(2)  
...
c           write(*,*)'rside1 zplus,hp',zplus,hp
c           write(*,*)'rside1 zminus,hp',zminus,hm
c           write(*,*)'rside1 dddz deru(4)',dddz,deru(4)     
     ...
     5  	   hm)/(2.d0*hr)
           deru(5)=-dddr
c
c           write(*,*)'rside1 rplus,hp',rplus,hp
c           write(*,*)'rside1 rminus,hp',rminus,hm
c           write(*,*)'rside1 dddr deru(5)',dddr,deru(5)     
...
c           write(*,*)'rside1 phipls,hp',phipls,hp
c           write(*,*)'rside1 phimin,hp',phimin,hm
c           write(*,*)'rside1 dddphi deru(6)',dddr,deru(6)     
c-----------------------------------------------------------
	   bmod=b(z,r,phi)  
...
cSmirnov961217 test
	  cnparp=(cnzplus*bz+cnrplus*br+cmplus*bphi/r)/bmod
	  cnperp=dsqrt(cnzplus**2+cnrplus**2+(cmplus/r)**2-cnparp**2)
c	  write(*,*)'cnrplus,cnzplus,cmplus',cnrplus,cnzplus,cmplus
c	  write(*,*)'in rside1 before 1control cnparp,cnperp',
c     1	  cnparp,cnperp
c	  call control(cnparp,cnperp)
c	  write(*,*)'in rside1 after 1control'
cSmirnov961217 test
c*************************************************
c----------------------------------------------------------
...
cSmirnov961217 test
	  cnparm=(cnzminus*bz+cnrminus*br+cmminus*bphi/r)/bmod
	  cnperm=dsqrt(cnzminus**2+cnrminus**2+(cmminus/r)**2-cnparm**2)
c	write(*,*)'cnrminus,cnzminus,cmminus',cnrminus,cnzminus,cmminus
c	  write(*,*)'in rside1 before 2control cnparm,cnperm',
c     1	  cnparm,cnperm
c          call control(cnparm,cnperm,reps)
c	  write(*,*)'in rside1 after 2control'
cSmirnov961217 test
c*************************************************
	   dddw=(hp-hm)/(2.0d0*hw)
c	   dddw=(hp-hm)/(2.0d0*hfrqnc)
c-----------------------------------------------------------
          do 14 i=1,nbulk
	  v(i)=vp(i)
	  w(i)=wp(i)
 14       continue
          p=-1.d0/dddw
cSmirnov961212
c	  write(*,*)'in rside1 hp,hm,hw,dddw'
c	  write(*,*)hp,hm,hw,dddw
c          write(*,*)'rside1 numerical deriv'
...
c
c          
c 	  p=1.d0/dsqrt(deru(1)*deru(1)+deru(2)*deru(2))
c     &   *sign  

c          write(*,*)'i_geom_optic ',i_geom_optic 

           if (i_geom_optic.eq.2) then
c          if ((id.eq.4).or.(id.eq.10)) then
...
c              write(*,*)'p,ray_direction',p,ray_direction
              p=ray_direction*p
...
c 	  write(*,*)'p',p
c	  write(*,*)'deru3',deru(1),deru(2),deru(3)
c	  write(*,*)'deru6', deru(4),deru(5),deru(6)
c	  write(*,*)'dddw',dddw
c-----------------------------------------------------------
cSAP111119 test
c          go to 1953
...
c-----------------------------------------------------------------
           if (((id.eq.4).or.(id.eq.5)).or.
     &         ((id.eq.7).or.(id.eq.10))) then
             write(*,*)'rside1: the given dispersion relation id=',id
             write(*,*)'does not work with the analytical derivatives'
             stop
           endif
c           write(*,*)'rside1 1955 z,r,phi', z,r,phi
c           write(*,*)'rside1 1955 cnz,cnr,cm', cnz,cnr,cm
...
c         write(*,*)'after s:s1,s2,s3,s4,s6,s7',s1,s2,s3,s4,s6,s7
...
c         write(*,*)'rside1 z,r,phi,ib,xib,yib',z,r,phi,ib,xib,yib
...
c       write(*,*)'rside1 z,r,phi 1,xe,ye',z,r,phi,xe,ye
...
c         write(*,*)'i,dxidz,dxidr,dxidph',
c     &              i,dxidz,dxidr,dxidph

         dyidz=dydz(z,r,phi,i)
         dyidr=dydr(z,r,phi,i)
         dyidph=dydphi(z,r,phi,i)

c         write(*,*)'i,dyidz,dyidr,dyidph',
c     &              i,dyidz,dyidr,dyidph
c--------------------------------------------
cc test
...
c         write(*,*)'i.ne.ib i,xi,yi',i,xi,yi
...
c--------------------------------------------
cc test
cc	 write(*,*)'in 30 rzide1 dadz,dadr,dadphi',
cc     1	 dadz,dadr,dadphi
cc	 write(*,*)'in 30 rzide1 dbdz,dbdr,dbdphi',
cc     1	 dbdz,dbdr,dbdphi
cc	 write(*,*)'in 30 rzide1 dcdz,dcdr,dcdphi',
cc     1	 dadz,dadr,dadphi
cc test
c--------------------------------------------
c new-------------------------------------
...
c if 11 end
 ...
 40     continue
         ...
         dyidr=dydr(z,r,phi,1)
         dyidph=dydphi(z,r,phi,1)
c         write(*,*)'1 dxidz,dxidr,dxidph',
c     &                dxidz,dxidr,dxidph
c------------------------------------------------------------------
...
c--------------------------------------------
cc test
cc	 write(*,*)'in 40 rzide1 dxidz,dxidr,dxidph',
cc     1	 dxidz,dxidr,dxidph
cc	 write(*,*)'in rzide1 dyidz,dyidr,dyidph',
cc     1	 dyidz,dyidr,dyidph
cc test
...
c--------------------------------------------
c test
cc	 write(*,*)'in 40 rzide1 dadz,dadr,dadphi',
cc     1   dadz,dadr,dadphi
cc	 write(*,*)'in 30 rzide1 dbdz,dbdr,dbdphi',
cc     1	 dbdz,dbdr,dbdphi
cc	 write(*,*)'in 30 rzide1 dcdz,dcdr,dcdphi',
...
c         write(*,*)'rside1 before hotderiv'
c	    write(*,*)'u1-3',u(1),u(2),u(3)
c	    write(*,*)'u4-6',u(4),u(5),u(6)

c	    write(*,*)'wf',frqncy

c          write(*,*)'rside1.f before  hotderiv'
c           call hotderiv(u,frqncy,dddcnz,dddcnr,dddcm,
c     .                   dddz,dddr,dddphi,dddw)

c            write(*,*)'rside1 after hotderv'
c 	    write(*,*)'dddcnz,dddcnr,dddcm',dddcnz,dddcnr,dddcm
c	    write(*,*)'dddz,dddr,dddphi', dddz,dddr,dddphci
c	    write(*,*)'dddw',dddw

c          write(*,*)'rside1.f before  hotderivs'
c          write(*,*)'rside1 u(1-6)'
c          write(*,*)u

c          write(*,*)'rside1.f before  hotdervs'
          call hotdervs(nbulk,u,wf,dddcnz,dddcnr,dddcm,
     .                  dddz,dddr,dddphi,dddw,ifsd)

c	    write(*,*)'rside1 after hotdervs'
c 	    write(*,*)'dddcnz,dddcnr,dddcm',dddcnz,dddcnr,dddcm
c	    write(*,*)'dddz,dddr,dddphi', dddz,dddr,dddphi
c	    write(*,*)'dddw',dddw
c	stop
...
c-----------------------------------------------------------------
        if (id.eq.8)then
c-----------------------------------------------------------------
c       Ono hot plasma dispersion for fast waves
c-----------------------------------------------------------------

c         write(*,*)'rside1 before ono_dervs'
c	    write(*,*)'u1-3',u(1),u(2),u(3)
c	    write(*,*)'u4-6',u(4),u(5),u(6)

c	    write(*,*)'wf',frqncy

c          write(*,*)'rside1.f before  ono_dervs'
c          write(*,*)'rside1 u(1-6)'
c          write(*,*)u
...
c	    write(*,*)'rside1 after ono_dervs'
c 	    write(*,*)'dddcnz,dddcnr,dddcm',dddcnz,dddcnr,dddcm
c	    write(*,*)'dddz,dddr,dddphi', dddz,dddr,dddphi
c	    write(*,*)'dddw',dddw
c	stop
...
c       end of Ono hot plasma dispersion for fast waves
c---------------------------------------------------------------
        if (id.eq.16)then
c          Paul T. Bonoli and Ronald C. Englade   dispersion
c          for LH  wave: asymptotic cold plasma plus thernmal correction    
c-----------------------------------------------------------------
...
c----------test      numericle derivative
c           write(*,*)'analitical dddcnz,dddcnr,dddcm',
c     &                dddcnz,dddcnr,dddcm  

c           write(*,*)'analitical dddz,dddr,dddphi,dddw',
c     &                           dddz,dddr,dddphi,dddw

c           write(*,*)'rside1 before num_der_d_d0_d_nr_nz_cm'
c           call num_der_d_d0_d_nr_nz_cm(u,nbulk,wf)
c           write(*,*)'rside1 after num_der_d_d0_d_nr_nz_cm'
           goto 50
        endif !id.eq.16
c----------------------------------------------------------------------
 50     continue
...
1953   continue
c test beg
c	   write(*,*)'in rside1 idif',idif
c	   write(*,*)'u3 ',u(1),u(2),u(3)
c	   write(*,*)'u6 ',u(4),u(5),u(6)
c         write(*,*)'deru3 ',deru(1),deru(2),deru(3)
c         write(*,*)'deru6 ',deru(4),deru(5),deru(6)
c         write(*,*)'dddz,dddr,dddphi',dddz,dddr,dddphi
c         write(*,*)'dddcnz,dddcnr,dddcm',dddcnz,dddcnr,dddcm
c	 write(*,*)'rside1 dddw ',dddw
c         stop 'rside1 test'
c test end
