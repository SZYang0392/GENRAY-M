...
c      beta_p2=0.d0
      ...
      k4 = 4.193828904d7                   !constant for elec therm vel
...
c-------------------------------------------------------------------
c     electron terms
      eps_perp=1.d0-X_ar(1)/(1-y_ar(1)**2)  
      eps_par=1.d0-x_ar(1)
      eps_xy=x_ar(1)*y_ar(1)/(1-y_ar(1)**2) 
c     ion terms
      do i=2,nbulk
         eps_perp=eps_perp-x_ar(i)/(1-y_ar(i)**2) 
         eps_par=eps_par-x_ar(i)
         eps_xy=eps_xy+x_ar(i)*y_ar(i)/(1-y_ar(i)**2) 
      enddo
...
c--------------------------------------------------------------
      ...
      c = 2.99792458d10                    !speed of light
      k4 = 4.193828904d7                   !constant for elec therm v
...
c-----plus ion's thermal term
      ...
      enddo
      ! For slowing-down ions
      vc_sd = (0.75d0*1.772453851d0*vc_sd/X_ar(1))**(1.d0/3.d0)*ve_sd
      vt_dc = vc_sd/c
      do i=2,nbulk 
            if (ifsd(i) >= 0.5d0) then
                  P_6 = P_6 - (3.d0/2.d0)*X_ar(i)*vt_dc**2
...
!          endif
         d_eps_perp_d_x_ar(i) = -1.d0/(1-y_ar(i)**2) 
         d_eps_par_d_x_ar(i)  = -1.d0
